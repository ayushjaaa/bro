-- Cart tracking was writable by ANYONE, with no login at all. Run after 020.
--
-- Found by probing every table and RPC as (a) anon and (b) a freshly signed-up, non-admin user:
--   * `cart_events` had "Anyone can log cart activity" with `check (true)` -> any visitor could insert
--     unlimited rows with any customer_id / product_id / action / quantity.
--   * `upsert_cart_snapshot_batch(p_customer_id, p_items)` is SECURITY DEFINER and was granted to
--     `anon`. It first DELETES every cart_snapshot row for p_customer_id and then inserts whatever
--     jsonb the caller sends -- so anyone could wipe or overwrite any customer's cart record, or bloat
--     the table, with no size limit.
-- Both feed the admin panel's cart / funnel analytics (sales reps chase abandoned carts from them), so
-- this is unauthenticated tampering with data staff act on, plus a storage-abuse vector.
--
-- The storefront only ever writes these from the SERVER, with the signed-in customer's own session
-- (lib/cart-tracking.ts -> reportCartActivity, called with `access.customer.id`, approved customers
-- only). So: require that session, and require that the customer_id being written is the caller's own
-- approved customer row.
--
-- Follows Supabase's RLS guidance: name the role (`to authenticated`), `with check` for inserts,
-- `(select auth.uid())` so it is evaluated once per statement, and for SECURITY DEFINER functions set
-- `search_path`, revoke from public/anon and grant only the roles that need it.

begin;

-- 1) cart_events: only an approved customer may log activity, only for themselves, only known actions.
drop policy if exists "Anyone can log cart activity" on cart_events;
drop policy if exists "Approved customers can log their own cart activity" on cart_events;

create policy "Approved customers can log their own cart activity" on cart_events for insert
  to authenticated
  with check (
    action in ('add_to_cart', 'update_quantity', 'remove_from_cart')
    and (quantity is null or quantity between 0 and 100000)
    and exists (
      select 1 from customers c
      where c.id::text = cart_events.customer_id
        and c.supabase_user_id = (select auth.uid())
        and c.status = 'approved'
    )
  );

-- 2) upsert_cart_snapshot_batch: same ownership rule, plus size/value limits, and no anon access.
create or replace function public.upsert_cart_snapshot_batch(p_customer_id text, p_items jsonb) returns void
language plpgsql
security definer
set search_path = ''
as $$
begin
  -- service_role (trusted server jobs) may write any cart; everyone else only their own approved cart.
  if (select auth.role()) <> 'service_role' and not exists (
    select 1 from public.customers c
    where c.id::text = p_customer_id
      and c.supabase_user_id = (select auth.uid())
      and c.status = 'approved'
  ) then
    raise exception 'not allowed' using errcode = '42501';
  end if;

  if p_items is null or jsonb_typeof(p_items) <> 'array' or jsonb_array_length(p_items) > 500 then
    raise exception 'invalid items' using errcode = '22023';
  end if;

  delete from public.cart_snapshot where customer_id = p_customer_id;

  -- group by variant: the table's primary key is (customer_id, variant_id), so a variant that appears
  -- twice in one payload is summed instead of failing the whole write.
  insert into public.cart_snapshot (customer_id, product_id, variant_id, quantity, updated_at)
  select p_customer_id,
         max(left(item->>'product_id', 200)),
         left(item->>'variant_id', 200),
         sum((item->>'quantity')::integer),
         now()
  from jsonb_array_elements(p_items) as item
  where (item->>'quantity')::integer between 1 and 100000
    and coalesce(item->>'variant_id', '') <> ''
  group by left(item->>'variant_id', 200);
end;
$$;

revoke all on function public.upsert_cart_snapshot_batch(text, jsonb) from public, anon, authenticated;
grant execute on function public.upsert_cart_snapshot_batch(text, jsonb) to authenticated, service_role;

commit;

-- ROLLBACK (only if something legitimate breaks; it re-opens the hole):
--   begin;
--   drop policy if exists "Approved customers can log their own cart activity" on cart_events;
--   create policy "Anyone can log cart activity" on cart_events for insert with check (true);
--   create or replace function public.upsert_cart_snapshot_batch(p_customer_id text, p_items jsonb) returns void
--   language plpgsql security definer set search_path = public as $f$
--   begin
--     delete from cart_snapshot where customer_id = p_customer_id;
--     insert into cart_snapshot (customer_id, product_id, variant_id, quantity, updated_at)
--     select p_customer_id, (item->>'product_id'), (item->>'variant_id'), (item->>'quantity')::integer, now()
--     from jsonb_array_elements(p_items) as item where (item->>'quantity')::integer > 0;
--   end; $f$;
--   grant execute on function public.upsert_cart_snapshot_batch(text, jsonb) to anon, authenticated, service_role;
--   commit;
